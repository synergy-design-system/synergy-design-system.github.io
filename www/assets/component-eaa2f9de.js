...
`:"",r+=`arg ref - \`${t}\``),r}const At=s=>`on${St(Tt(s))}`;function Tt(s=""){return s.split("-").map((r,n)=>n?r.charAt(0).toUpperCase()+r.slice(1).toLowerCase():r.toLowerCase()).join("")}function St(s){return s.charAt(0).toUpperCase()+s.slice(1)}const{useArgs:Ct}=__STORYBOOK_MODULE_CLIENT_API__;let D,W;var q;function Nt(s,t,e){if(!t)return h`<${y(s.tagName)}></${y(s.tagName)}>`;s?.tagName!==W&&(D=void 0,W=s?.tagName);const r=_t(s,t),n=Ot(s,t);Pt(s);const a=Et(s,t);return h(q||(q=F(["",`
<`," "," ",">","","</",`>

<script>
  component = document.querySelector('`,`');
<\/script>
`])),K(s,t),y(s.tagName),yt(r),a,n,e||"",y(s.tagName),s.tagName)}function K(s,t){const e=kt(s,t);return`${e}`?.replaceAll(/\s+/g,"")!=""?h`<style>${e}</style>
`:""}function _t(s,t){const e=M(s),r={};return Object.keys(e).filter(n=>n.endsWith("-attr")).forEach(n=>{const a=e[n],o=e[n].name,c=t[n];if(c){const g=a.control.type==="boolean"?`?${o}`:o;r[g]=c==="false"?!1:c}}),Object.keys(t).filter(n=>!n.endsWith("-attr")&&!n.endsWith("-part")&&!n.endsWith("-slot")).forEach(n=>{if(n.startsWith("on"))return;const a=t[n];a&&(r[`.${n}`]=a)}),r}function Et(s,t){const e=I(s);return Object.keys(e).some(a=>!!t[a])?y(`style="${Object.keys(e).map(a=>{const o=e[a].name,c=t[a];return c?`${o}: ${c||""}`:null}).filter(a=>a!==null).join(";")}"`):void 0}function kt(s,t){const e=z(s);return Object.keys(e).some(a=>!!t[a])?y(`${Object.keys(e).filter(a=>a.endsWith("-part")).map(a=>{const o=e[a].name,c=t[a];return c?.replaceAll(/\s+/g,"")!==`${s?.tagName}::part(${o}){}`?`
${c}`:null}).filter(a=>a!==null).join(`
`)}
`):void 0}function Ot(s,t){const e=Q(s);return y(`${Object.keys(e).filter(n=>n.endsWith("-slot")).map(n=>{const a=e[n].name,o=t[n];return a==="default"?o||null:o!==`<span slot="${a}"></span>`?o:null}).filter(n=>n!==null).join("")}`)}function Pt(s){Dt(s),setTimeout(()=>{const t=document.querySelector(s.tagName);D?.observe(t,{attributes:!0})})}function Dt(s){let t=!1;const e=Ct()[1],r=M(s);D||(D=new MutationObserver(n=>{n.forEach(a=>{if(a.type!=="attributes"||a.attributeName==="class"&&t)return;t=!0;const o=r[`${a.attributeName}-attr`];o?.control==="boolean"||o?.control?.type==="boolean"?e({[`${a.attributeName}-attr`]:a.target?.hasAttribute(a.attributeName||"")}):e({[`${a.attributeName}-attr`]:a.target.getAttribute(a.attributeName||"")}),t=!1})}))}function x(s,t){const e=window.__STORYBOOK_CUSTOM_ELEMENTS_MANIFEST__;if(!e)throw new Error(`Custom Elements Manifest not found. Be sure to follow the pre-install steps in this guide:
https://www.npmjs.com/package/wc-storybook-helpers#before-you-install`);const r=vt(s,e),n=r?.events?.map(a=>a.name)||[];return{argTypes:X(r,{showArgRef:t?.showArgRef}),reactArgTypes:jt(r),args:Lt(r),events:n,styleTemplate:a=>K(r,a),template:(a,o)=>Nt(r,a,o),manifest:r}}function X(s,t){return{...M(s,{showArgRef:t?.showArgRef}),...Q(s,{showArgRef:t?.showArgRef}),...I(s),...z(s,{showArgRef:t?.showArgRef})}}function Lt(s){return Object.entries(X(s)).filter(([,e])=>e?.control).map(([e,r])=>({[e]:r.defaultValue||""})).reduce((e,r)=>({...e,...r}),{})}function jt(s){return{...bt(s),...wt(s)}}const U=String.raw`[A-Za-z][^/\s>]*`,Mt=String.raw`(?<!\w)"(?:\\[^<>\n]|[^\\"<>\n])*"(?!\w)`,Rt=String.raw`(?<!\w)'(?:\\[^<>\n]|[^\\'<>\n])*'(?!\w)`,Z=String.raw`${Mt}|${Rt}`,Vt=String.raw`"(?<quotedAttrValue>[^"]*)"`,Ft=String.raw`'(?<singleQuotedAttrValue>[^']*)'`,Wt=String.raw`(?<unquotedAttrValue>[^\s"'\`=<>]+)`,Ut=String.raw`[^=\s>/"']+(?=[=>\s]|$)`,qt=String.raw`${Vt}|${Ft}|${Wt}`,Bt=String.raw`(?<attrName>${Ut})(?:\s*=\s*(?:${qt}))?`,It=String.raw`${Z}|[^\s>]*[^\s>/]|[^\s>]*/(?!\s*>)`,G=String.raw`(?<attrSpace>\s*)(?:${Bt}|(?<attrText>${It}))`,zt={comment:String.raw`<!--.*?-->`,dtd:String.raw`<![^>]+>`,startTag:String.raw`<(?<startTagName>${U})(?<attrs>(?:${G})*)\s*(?<closingSlash>/?)\s*>`,endTag:String.raw`</(?<endTagName>${U})\s*>`,space:String.raw`\s+`,text:String.raw`[^<\s"']+|${Z}|['"]`,wildcard:String.raw`.`},Qt=Object.entries(zt).map(([s,t])=>`(?<${s}>${t})`).join("|");function*Ht(s,t){let e,{lastIndex:r}=s;for(;e=s.exec(t);)yield e,{lastIndex:r}=s;if(r!=t.length)throw new Error("Failed to parse string")}const Yt=new Set(["area","base","basefont","bgsound","br","col","command","embed","frame","hr","image","img","input","keygen","link","meta","param","source","track","wbr"]);function L(s,t="  ",e=80){const r=new RegExp(Qt,"gys"),n=new RegExp(G,"gy"),a=[];let o=null,c=0,g=!1,v=0;const i=(...d)=>{for(const u of d){if(!o)if(u==`
`)g=!0;else{const N=u.indexOf(`
`),m=N==-1?u.length:N;v+m>e&&/^[ \t]+$/.test(a[a.length-1])&&(a.pop(),i(`
`)),g&&(g=!1,i(t.repeat(c)))}const A=u.lastIndexOf(`
`);A==-1?v+=u.length:v=u.length-A-1,a.push(u)}};for(const d of Ht(r,s)){if(L.__strict&&d.groups.wildcard)throw new Error("Unexpected wildcard");if(d.groups.endTag){const u=d.groups.endTagName.toLowerCase();u==o&&(o=null),o||(--c,i(`</${u}>`))}if(o)i(d[0]);else if(d.groups.space)i(...d[0].match(/\n/g)?.slice(0,2)??[" "]);else if(d.groups.comment||d.groups.dtd||d.groups.text||d.groups.wildcard)i(d[0]);else if(d.groups.startTag){const u=d.groups.startTagName.toLowerCase();if(i(`<${u}`),++c,d.groups.attrs){let{lastIndex:N}=n,m,S;for(;m=n.exec(d.groups.attrs);){if({lastIndex:N}=n,L.__strict&&m.groups.attrText)throw new Error("Unexpected attr text");m.groups.attrText?(m.groups.attrSpace&&i(/\n/.test(m.groups.attrSpace)?`
`:" "),i(m.groups.attrText)):((m.groups.attrSpace||!S?.groups.attrText)&&i(/\n/.test(m.groups.attrSpace)?`
`:" "),i(`${m.groups.attrName}${m.groups.quotedAttrValue?`="${m.groups.quotedAttrValue}"`:m.groups.singleQuotedAttrValue?`='${m.groups.singleQuotedAttrValue}'`:m.groups.unquotedAttrValue?`=${m.groups.unquotedAttrValue}`:""}`)),S=m}if(N!=d.groups.attrs.length)throw new Error("Failed to parse attributes")}const A=!!d.groups.closingSlash;i(A?" />":">"),A||Yt.has(u)?--c:["pre","script","style"].includes(u)&&(o=u)}}let $=!1;for(;/^\s+$/.test(a[a.length-1]);){const d=a.pop();/\n/.test(d)&&($=!0)}return $&&a.push(`
`),a.join("")}L.default=L;function Kt(s){return s.charAt(0).toUpperCase()+s.substr(1)}function xt(s,t){var e=s.toLowerCase();return t===0?Kt(e):e}function Xt(s,t){return t===void 0&&(t={}),lt(s,it({delimiter:" ",transform:xt},t))}async function Zt(){await fetch("./custom-elements.json");const t=await(await fetch("./custom-elements.json")).json();ot(t),console.log("Custom elements manifest loaded")}await Zt();const j=s=>{const{args:t,events:e,argTypes:r,manifest:n}=x(s),a=Object.fromEntries(Object.entries(r).map(([g,v])=>v?.control?.type==="number"&&v.options&&v.options?.length>0?[g,{...v,control:{type:"select"}}]:[g,v])),o=()=>({status:{styles:{backgroundColor:n?.status==="stable"?"#43b02a":"rgb(255, 131, 0)",borderColor:"white",color:"white"},title:`Status: ${n?.status}`},since:{styles:{backgroundColor:"#333",borderColor:"#fff",color:"#fff"},title:`Since: ${n?.since}`}});return{args:t,argTypes:(()=>{const g=()=>{const i=n?.members?.filter(u=>u.kind==="field"),$=new Set(n?.attributes?.map(u=>u.fieldName));return i?.filter(u=>!$.has(u.name)&&u?.privacy!=="private")?.map(u=>u.name)};return{...a,...n?.events?.reduce((i,$)=>(i[$.name]={...$,control:!1,table:{category:"Events"}},i),{}),...n?.members?.filter(i=>i.kind==="method"&&i?.privacy!=="private").reduce((i,$)=>(i[$.name]={...$,control:!1,table:{category:"Methods"}},i),{}),...g()?.reduce((i,$)=>(i[`${$}-prop`]={table:{disable:!0}},i[$]={control:!1,table:{category:"Properties"}},i),{})}})(),parameters:{badges:["status","since"],badgesConfig:o(),actions:{handles:e}}}},_=s=>({getSuffixFromType:t=>({attribute:"-attr",property:"-prop",slot:"-slot",cssPart:"-part",cssProperty:""})[t],getValuesFromAttribute:t=>{t.endsWith("-attr")||(t=`${t}-attr`);const{argTypes:e}=j(s);return e[t]?.control?.type==="boolean"?[!0,!1]:e[t].options},getValuesFromAttributes:t=>t?.map(e=>(e.endsWith("-attr")||(e=`${e}-attr`),{name:e,values:_(s).getValuesFromAttribute(e)})),overrideArgs:(t,e)=>{const r=e||j(s).args,n=Array.isArray(t)?t:[t];for(const a of n){const o=_(s).getSuffixFromType(a.type);r[`${a.name}${o}`]=a.value}return r}}),ae=s=>{const{template:t,manifest:e}=x(s),{args:r}=j(s),{getValuesFromAttribute:n}=_(s);return{generateTemplate:({axis:o,constants:c=[],options:g,args:v=r})=>{const i=l=>{if(!e?.style)return t(l);const p=Object.keys(l).filter(f=>f.endsWith("-attr")).reduce((f,w)=>{const b=w.substring(0,w.length-5);return l[w]===!0?f[b]=!0:l[w]==="false"||l[w]==="(default)"?f[b]=!1:l[w]&&(b.endsWith("...")?f[b.replace("...",l[w])]=!0:f[l[w]]=!0),f},{}),T=l["default-slot"]||"",C={[s]:!0,...p},E=Object.keys(C).filter(f=>C[f]).join(" ");if(g?.templateContent){const f=g.templateContent.replaceAll("%SLOT%",T).replaceAll("%CLASSES%",E);return y(f)}return h`<div class=${st(C)}>${y(T)}</div>`},$=(Array.isArray(c)?c:[c]).reduce((l,p)=>({...l,[`${p.name}${_(s).getSuffixFromType(p.type)}`]:p.value}),{});if(!o?.x&&!o?.y&&!g?.title)return h`${i({...v,...$})}`;const d=l=>l?Array.isArray(l)?l.map(p=>({...p,values:p.values||n(p.name)})):[{...l,values:l.values||n(l.name)}]:[{}],u=d(o?.x),A=d(o?.y),m=(Array.isArray(c)?c:[c]).find(l=>l.type==="template")?.value,S=`uuid-${crypto.randomUUID()}`;return h`
      <style>
        ...
        ${g?.templateBackgrounds?.colors.map((l,p)=>{const T=C=>`${g?.templateBackgrounds?.colors.length}n + ${C+1}`;return g?.templateBackgrounds?.alternate==="y"?`
      ...
      ${u.map(l=>h` ${A.map(p=>{let T=!0;const C=u.length>1||l.values,E=(l&&p||A.length>1)&&p?.values;return h`
            <table class="story-template ${S} ${g?.classes}">
              <thead>
                ...
                ${l&&l.values&&h`
                  <tr>
                    ${E?h`<td></td>`:""} <td></td>
                    ${C&&h`<th colspan=${l.values?.length||0}><code>${l.title||l.name}</code></th>`}
                    </tr>
                  </tr>
                  ${h`
                    <tr>
              ...
              <tbody>
                ${(p?.values||[""]).map(f=>{const w=h`
                    <tr class="template-row">
                      ${T&&E?h`<th rowspan="${p?.values?.length}">
                      ...
                      ${(l?.values||[""]).map(b=>h`
                          ...
                          ${m&&y(m.split("%TEMPLATE%")[0]||"")||""}
                            ${i({...v,...$,...l&&l.type!=="template"&&{[`${l.name}${_(s).getSuffixFromType(l.type)}`]:b.hasOwnProperty("value")?b.value:b},...p&&p.type!=="template"&&{[`${p.name}${_(s).getSuffixFromType(p.type)}`]:f.hasOwnProperty("value")?f.value:f}})}
          ...
          `})}`)}
    `}}},oe=(s,t)=>ct?.components?.[s]?.[t]?.description?.value??"No Description",le=(s,t=150)=>({parameters:{chromatic:{...ut?.parameters?.chromatic,disableSnapshot:!1},docs:{disable:!0}},render:(e,r)=>h`${Object.entries(s).map(([n,a])=>{const o=Xt(n);return h`
    <div style='height: ${t}px; margin: var(--syn-spacing-small)'>
      <h3 data-chromatic="ignore">${o}</h3>
      ${a.render?.(e,r)}
    </div>
    `})}`});export{ae as a,le as b,_ as c,oe as g,j as s};
