...
`:"",s+=`arg ref - \`${t}\``),s}const ue=n=>`on${de(pe(n))}`;function pe(n=""){return n.split("-").map((s,r)=>r?s.charAt(0).toUpperCase()+s.slice(1).toLowerCase():s.toLowerCase()).join("")}function de(n){return n.charAt(0).toUpperCase()+n.slice(1)}const{useArgs:he}=__STORYBOOK_MODULE_CLIENT_API__;let q,ht;var $t;function fe(n,t,e){if(!t)return J`<${H(n.tagName)}></${H(n.tagName)}>`;n?.tagName!==ht&&(q=void 0,ht=n?.tagName);const s=$e(n,t),r=ve(n,t);Ae(n);const o=ge(n,t);return J($t||($t=rt(["",`
<`," "," ",">","","</",`>

<script>
  component = document.querySelector('`,`');
<\/script>
`])),Et(n,t),H(n.tagName),re(s),o,r,e||"",H(n.tagName),n.tagName)}function Et(n,t){const e=me(n,t);return`${e}`?.replaceAll(/\s+/g,"")!=""?J`<style>${e}</style>
`:""}function $e(n,t){const e=et(n),s={};return Object.keys(e).filter(r=>r.endsWith("-attr")).forEach(r=>{const o=e[r],i=e[r].name,u=t[r];if(u){const a=o.control.type==="boolean"?`?${i}`:i;s[a]=u==="false"?!1:u}}),Object.keys(t).filter(r=>!r.endsWith("-attr")&&!r.endsWith("-part")&&!r.endsWith("-slot")).forEach(r=>{if(r.startsWith("on"))return;const o=t[r];o&&(s[`.${r}`]=o)}),s}function ge(n,t){const e=bt(n);return Object.keys(e).some(o=>!!t[o])?H(`style="${Object.keys(e).map(o=>{const i=e[o].name,u=t[o];return u?`${i}: ${u||""}`:null}).filter(o=>o!==null).join(";")}"`):void 0}function me(n,t){const e=wt(n);return Object.keys(e).some(o=>!!t[o])?H(`${Object.keys(e).filter(o=>o.endsWith("-part")).map(o=>{const i=e[o].name,u=t[o];return u?.replaceAll(/\s+/g,"")!==`${n?.tagName}::part(${i}){}`?`
${u}`:null}).filter(o=>o!==null).join(`
`)}
`):void 0}function ve(n,t){const e=Tt(n);return H(`${Object.keys(e).filter(r=>r.endsWith("-slot")).map(r=>{const o=e[r].name,i=t[r];return o==="default"?i||null:i!==`<span slot="${o}"></span>`?i:null}).filter(r=>r!==null).join("")}`)}function Ae(n){ye(n),setTimeout(()=>{const t=document.querySelector(n.tagName);q?.observe(t,{attributes:!0})})}function ye(n){let t=!1;const e=he()[1],s=et(n);q||(q=new MutationObserver(r=>{r.forEach(o=>{if(o.type!=="attributes"||o.attributeName==="class"&&t)return;t=!0;const i=s[`${o.attributeName}-attr`];i?.control==="boolean"||i?.control?.type==="boolean"?e({[`${o.attributeName}-attr`]:o.target?.hasAttribute(o.attributeName||"")}):e({[`${o.attributeName}-attr`]:o.target.getAttribute(o.attributeName||"")}),t=!1})}))}function Ct(n,t){const e=window.__STORYBOOK_CUSTOM_ELEMENTS_MANIFEST__;if(!e)throw new Error(`Custom Elements Manifest not found. Be sure to follow the pre-install steps in this guide:
https://www.npmjs.com/package/wc-storybook-helpers#before-you-install`);const s=ae(n,e),r=s?.events?.map(o=>o.name)||[];return{argTypes:kt(s,{showArgRef:t?.showArgRef}),reactArgTypes:be(s),args:_e(s),events:r,styleTemplate:o=>Et(s,o),template:(o,i)=>fe(s,o,i),manifest:s}}function kt(n,t){return{...et(n,{showArgRef:t?.showArgRef}),...Tt(n,{showArgRef:t?.showArgRef}),...bt(n),...wt(n,{showArgRef:t?.showArgRef})}}function _e(n){return Object.entries(kt(n)).filter(([,e])=>e?.control).map(([e,s])=>({[e]:s.defaultValue||""})).reduce((e,s)=>({...e,...s}),{})}function be(n){return{...le(n),...ce(n)}}const ft=String.raw`[A-Za-z][^/\s>]*`,we=String.raw`(?<!\w)"(?:\\[^<>\n]|[^\\"<>\n])*"(?!\w)`,Te=String.raw`(?<!\w)'(?:\\[^<>\n]|[^\\'<>\n])*'(?!\w)`,Ot=String.raw`${we}|${Te}`,Se=String.raw`"(?<quotedAttrValue>[^"]*)"`,Ne=String.raw`'(?<singleQuotedAttrValue>[^']*)'`,Ee=String.raw`(?<unquotedAttrValue>[^\s"'\`=<>]+)`,Ce=String.raw`[^=\s>/"']+(?=[=>\s]|$)`,ke=String.raw`${Se}|${Ne}|${Ee}`,Oe=String.raw`(?<attrName>${Ce})(?:\s*=\s*(?:${ke}))?`,He=String.raw`${Ot}|[^\s>]*[^\s>/]|[^\s>]*/(?!\s*>)`,Ht=String.raw`(?<attrSpace>\s*)(?:${Oe}|(?<attrText>${He}))`,Pe={comment:String.raw`<!--.*?-->`,dtd:String.raw`<![^>]+>`,startTag:String.raw`<(?<startTagName>${ft})(?<attrs>(?:${Ht})*)\s*(?<closingSlash>/?)\s*>`,endTag:String.raw`</(?<endTagName>${ft})\s*>`,space:String.raw`\s+`,text:String.raw`[^<\s"']+|${Ot}|['"]`,wildcard:String.raw`.`},Me=Object.entries(Pe).map(([n,t])=>`(?<${n}>${t})`).join("|");function*Le(n,t){let e,{lastIndex:s}=n;for(;e=n.exec(t);)yield e,{lastIndex:s}=n;if(s!=t.length)throw new Error("Failed to parse string")}const xe=new Set(["area","base","basefont","bgsound","br","col","command","embed","frame","hr","image","img","input","keygen","link","meta","param","source","track","wbr"]);function Q(n,t="  ",e=80){const s=new RegExp(Me,"gys"),r=new RegExp(Ht,"gy"),o=[];let i=null,u=0,a=!1,c=0;const l=(...p)=>{for(const f of p){if(!i)if(f==`
`)a=!0;else{const _=f.indexOf(`
`),m=_==-1?f.length:_;c+m>e&&/^[ \t]+$/.test(o[o.length-1])&&(o.pop(),l(`
`)),a&&(a=!1,l(t.repeat(u)))}const b=f.lastIndexOf(`
`);b==-1?c+=f.length:c=f.length-b-1,o.push(f)}};for(const p of Le(s,n)){if(Q.__strict&&p.groups.wildcard)throw new Error("Unexpected wildcard");if(p.groups.endTag){const f=p.groups.endTagName.toLowerCase();f==i&&(i=null),i||(--u,l(`</${f}>`))}if(i)l(p[0]);else if(p.groups.space)l(...p[0].match(/\n/g)?.slice(0,2)??[" "]);else if(p.groups.comment||p.groups.dtd||p.groups.text||p.groups.wildcard)l(p[0]);else if(p.groups.startTag){const f=p.groups.startTagName.toLowerCase();if(l(`<${f}`),++u,p.groups.attrs){let{lastIndex:_}=r,m,E;for(;m=r.exec(p.groups.attrs);){if({lastIndex:_}=r,Q.__strict&&m.groups.attrText)throw new Error("Unexpected attr text");m.groups.attrText?(m.groups.attrSpace&&l(/\n/.test(m.groups.attrSpace)?`
`:" "),l(m.groups.attrText)):((m.groups.attrSpace||!E?.groups.attrText)&&l(/\n/.test(m.groups.attrSpace)?`
`:" "),l(`${m.groups.attrName}${m.groups.quotedAttrValue?`="${m.groups.quotedAttrValue}"`:m.groups.singleQuotedAttrValue?`='${m.groups.singleQuotedAttrValue}'`:m.groups.unquotedAttrValue?`=${m.groups.unquotedAttrValue}`:""}`)),E=m}if(_!=p.groups.attrs.length)throw new Error("Failed to parse attributes")}const b=!!p.groups.closingSlash;l(b?" />":">"),b||xe.has(f)?--u:["pre","script","style"].includes(f)&&(i=f)}}let d=!1;for(;/^\s+$/.test(o[o.length-1]);){const p=o.pop();/\n/.test(p)&&(d=!0)}return d&&o.push(`
`),o.join("")}Q.default=Q;async function De(){await fetch("./custom-elements.json");const t=await(await fetch("./custom-elements.json")).json();Lt(t),console.log("Custom elements manifest loaded")}await De();const tt=n=>{const{args:t,events:e,argTypes:s,manifest:r}=Ct(n),o=Object.fromEntries(Object.entries(s).map(([a,c])=>c?.control?.type==="number"&&c.options&&c.options?.length>0?[a,{...c,control:{type:"select"}}]:[a,c])),i=()=>({status:{styles:{backgroundColor:r?.status==="stable"?"#43b02a":"rgb(255, 131, 0)",borderColor:"white",color:"white"},title:`Status: ${r?.status}`},since:{styles:{backgroundColor:"#333",borderColor:"#fff",color:"#fff"},title:`Since: ${r?.since}`}});return{args:t,argTypes:(()=>{const a=()=>{const l=r?.members?.filter(f=>f.kind==="field"),d=new Set(r?.attributes?.map(f=>f.fieldName));return l?.filter(f=>!d.has(f.name)&&f?.privacy!=="private")?.map(f=>f.name)};return{...o,...r?.events?.reduce((l,d)=>(l[d.name]={...d,control:!1,table:{category:"Events"}},l),{}),...r?.members?.filter(l=>l.kind==="method"&&l?.privacy!=="private").reduce((l,d)=>(l[d.name]={...d,control:!1,table:{category:"Methods"}},l),{}),...a()?.reduce((l,d)=>(l[`${d}-prop`]={table:{disable:!0}},l[d]={control:!1,table:{category:"Properties"}},l),{})}})(),parameters:{badges:["status","since"],badgesConfig:i(),actions:{handles:e}}}},M=n=>({getSuffixFromType:t=>({attribute:"-attr",property:"-prop",slot:"-slot",cssPart:"-part",cssProperty:""})[t],getValuesFromAttribute:t=>{t.endsWith("-attr")||(t=`${t}-attr`);const{argTypes:e}=tt(n);return e[t]?.control?.type==="boolean"?[!0,!1]:e[t].options},getValuesFromAttributes:t=>t?.map(e=>(e.endsWith("-attr")||(e=`${e}-attr`),{name:e,values:M(n).getValuesFromAttribute(e)})),overrideArgs:(t,e)=>{const s=e||tt(n).args,r=Array.isArray(t)?t:[t];for(const o of r){const i=M(n).getSuffixFromType(o.type);s[`${o.name}${i}`]=o.value}return s}}),Ue=n=>{const{template:t,manifest:e}=Ct(n),{args:s}=tt(n),{getValuesFromAttribute:r}=M(n);return{generateTemplate:({axis:i,constants:u=[],options:a,args:c=s})=>{const l=h=>{if(!e?.style)return t(h);const $=Object.keys(h).filter(g=>g.endsWith("-attr")).reduce((g,w)=>{const y=w.substring(0,w.length-5);return h[w]===!0?g[y]=!0:h[w]==="false"||h[w]==="(default)"?g[y]=!1:h[w]&&(y.endsWith("...")?g[y.replace("...",h[w])]=!0:g[h[w]]=!0),g},{}),T=h["default-slot"]||"",C={[n]:!0,...$},D=Object.keys(C).filter(g=>C[g]).join(" ");if(a?.templateContent){const g=a.templateContent.replaceAll("%SLOT%",T).replaceAll("%CLASSES%",D);return S(g)}return A`<div class=${Mt(C)}>${S(T)}</div>`},d=(Array.isArray(u)?u:[u]).reduce((h,$)=>({...h,[`${$.name}${M(n).getSuffixFromType($.type)}`]:$.value}),{});if(!i?.x&&!i?.y&&!a?.title)return A`${l({...c,...d})}`;const p=h=>h?Array.isArray(h)?h.map($=>({...$,values:$.values||r($.name)})):[{...h,values:h.values||r(h.name)}]:[{}],f=p(i?.x),b=p(i?.y),m=(Array.isArray(u)?u:[u]).find(h=>h.type==="template")?.value,E=`uuid-${crypto.randomUUID()}`;return A`
      <style>
      ...
      ${f.map(h=>A` ${b.map($=>{let T=!0;const C=f.length>1||h.values,D=(h&&$||b.length>1)&&$?.values;return A`
          ...
          `})}`)}
    `}}},Be=(n,t)=>xt?.components?.[n]?.[t]?.description?.value??"No Description",We=(n,t=150)=>({parameters:{chromatic:{disableSnapshot:!1},docs:{disable:!0}},render:(e,s)=>A`${n.map(r=>A`
    <div style='height: ${t}px; margin: var(--syn-spacing-small)'>
      <h3>${r.name}</h3>
      ${r.render?.(e,s)}
    </div>
    `)}`});export{Ue as a,We as b,M as c,Be as g,tt as s};
//# sourceMappingURL=component-be2efc65.js.map
