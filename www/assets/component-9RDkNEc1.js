...
<\/script>`])),s.tagName)}
`}function q(s,e){const r=Et(s,e);return`${r}`?.replaceAll(/\s+/g,"")!=""?f`<style>${r}</style>`:""}function Ot(s,e,r){const t=k(s),n={},a={},i={};return Object.keys(t).forEach(c=>{const o=t[c];if(o?.table?.category!=="attributes")return;const d=o.name,h=e[c],$=o.control.type==="boolean"?`?${d}`:d;(h!==t[c].defaultValue||M.renderDefaultAttributeValues)&&(n[$]=h==="false"?!1:h)}),Object.keys(e).filter(c=>t[c]?.table?.category==="properties").forEach(c=>{if(c.startsWith("on"))return;const o=e[c];a[`.${c}`]=o}),Object.keys(e).filter(c=>!Object.keys(r||{}).includes(c)).forEach(c=>{!c.startsWith("on")&&typeof e[c]!="function"&&(i[c]=e[c])}),{attrOperators:n,propOperators:a,additionalAttrs:i}}function wt(s,e){if(!s?.cssProperties?.length)return;const r=D(s);return m(`style="${Object.keys(r).map(t=>{const n=r[t].name,a=e[t];return a?`${n}: ${a||""}`:null}).filter(t=>t!==null).join(";")}"`)}function Et(s,e){const r=L(s);if(!Object.keys(r).some(a=>!!e[a]))return;const n=Object.keys(r).filter(a=>a.endsWith("-part")).map(a=>{const i=r[a].name,c=e[a];return c?.replaceAll(/\s+/g,"")!==`${s?.tagName}::part(${i}){}`?`
${c}`:null}).filter(a=>a!==null).join(`
`);return n?m(`${n}
`):""}function Ct(s,e){const r=j(s);return m(`${Object.keys(r).filter(n=>n.endsWith("-slot")).map(n=>{const a=r[n].name,i=e[n];return a==="default"?i||null:i!==`<span slot="${a}"></span>`?i:null}).filter(n=>n!==null).join("")}`)}function St(s){_t(s),setTimeout(()=>{const e=document.querySelector(s.tagName);P?.observe(e,{attributes:!0})})}function _t(s){let e=!1;const r=vt()[1],t=k(s);P||(P=new MutationObserver(n=>{n.forEach(a=>{if(a.type!=="attributes"||a.attributeName==="class"&&e)return;e=!0;const i=t[`${a.attributeName}`];i?.control==="boolean"||i?.control?.type==="boolean"?r({[`${a.attributeName}`]:a.target?.hasAttribute(a.attributeName||"")}):r({[`${a.attributeName}`]:a.target.getAttribute(a.attributeName||"")}),e=!1})}))}function Nt(s){window.__WC_STORYBOOK_HELPERS_CONFIG__=s}function J(s){const e=window.__STORYBOOK_CUSTOM_ELEMENTS_MANIFEST__;if(!e)throw new Error(`Custom Elements Manifest not found. Be sure to follow the pre-install steps in this guide:
https://www.npmjs.com/package/wc-storybook-helpers#before-you-install`);const r=Y(s,e),t=r?.events?.map(a=>a.name)||[],n=Q(r);return{args:X(r),argTypes:n,reactArgs:At(r),reactArgTypes:Z(r),events:t,styleTemplate:a=>q(r,a),template:(a,i)=>Tt(r,a,i,n)}}function Q(s){return{...D(s),...L(s),...j(s),...k(s),...mt(s)}}function X(s,e){return e||(e=Q(s)),Object.entries(e).filter(([,t])=>t?.control).map(([t,n])=>{let a=Pt(n.defaultValue);return n.table?.category==="css properties"&&(a=a?.toString()),{[t]:a===void 0?"":a}}).reduce((t,n)=>({...t,...n}),{})}function Pt(s){try{return JSON.parse(s)}catch{return s}}function Z(s){return{...dt(s),...z(s),...D(s,!1),...L(s,!1),...j(s,!1)}}function At(s){const e=X(s,Z(s)),r=Object.entries(z(s)).map(([t])=>({[t]:()=>{}})).reduce((t,n)=>({...t,...n}),{});return{...e,...r}}function kt(s){return s.charAt(0).toUpperCase()+s.substr(1)}function Dt(s,e){var r=s.toLowerCase();return e===0?kt(r):r}function Lt(s,e){return e===void 0&&(e={}),ot(s,lt({delimiter:" ",transform:Dt},e))}Nt({hideArgRef:!0,hideScriptTag:!0});async function jt(){await fetch("./custom-elements.json");let e=await(await fetch("./custom-elements.json")).json();e.modules&&(e.modules=e.modules.map(r=>(r.declarations&&(r.declarations=r.declarations.map(t=>(t.members&&(t.members=t.members.filter(n=>n.description&&n.privacy!=="private")),t))),r))),at(e),console.log("Custom elements manifest loaded")}await jt();const A=s=>{const{args:e,events:r,argTypes:t}=J(s),n=Y(s,window.__STORYBOOK_CUSTOM_ELEMENTS_MANIFEST__),a=()=>({status:{styles:{backgroundColor:n?.status==="stable"?"#43b02a":"rgb(255, 131, 0)",borderColor:"white",color:"white"},title:`Status: ${n?.status}`},since:{styles:{backgroundColor:"#333",borderColor:"#fff",color:"#fff"},title:`Since: ${n?.since}`}});return{args:e,argTypes:(()=>{for(const o in t)t.hasOwnProperty(o)&&t[o].table&&t[o].table.category==="properties"&&(t[o].control={disabled:!0});return{...t,...n?.events?.reduce((o,d)=>(o[d.name]={...d,control:{disabled:!0},table:{category:"Events"}},o),{}),...n?.members?.filter(o=>o.kind==="method").reduce((o,d)=>(o[d.name]={...d,control:!1,table:{category:"Methods"}},o),{})}})(),parameters:{badges:["status","since"],badgesConfig:a(),actions:{handles:r}}}},T=s=>({getSuffixFromType:e=>({attribute:"",property:"-prop",slot:"-slot",cssPart:"-part",cssProperty:""})[e],getValuesFromAttribute:e=>{e.endsWith("-attr")||(e=`${e}-attr`);const{argTypes:r}=A(s);return r[e]?.control?.type==="boolean"?[!0,!1]:r[e].options},getValuesFromAttributes:e=>e?.map(r=>(r.endsWith("-attr")||(r=`${r}-attr`),{name:r,values:T(s).getValuesFromAttribute(r)})),overrideArgs:(e,r)=>{const t=r||A(s).args,n=Array.isArray(e)?e:[e];for(const a of n){const i=T(s).getSuffixFromType(a.type);t[`${a.name}${i}`]=a.value}return t}}),It=s=>{const{template:e,manifest:r}=J(s),{args:t}=A(s),{getValuesFromAttribute:n}=T(s);return{generateTemplate:({axis:i,constants:c=[],options:o,args:d=t})=>{const h=l=>{if(!r?.style)return e(l);const u=Object.keys(l).filter(p=>p.endsWith("-attr")).reduce((p,y)=>{const g=y.substring(0,y.length-5);return l[y]===!0?p[g]=!0:l[y]==="false"||l[y]==="(default)"?p[g]=!1:l[y]&&(g.endsWith("...")?p[g.replace("...",l[y])]=!0:p[l[y]]=!0),p},{}),b=l["default-slot"]||"",v={[s]:!0,...u},w=Object.keys(v).filter(p=>v[p]).join(" ");if(o?.templateContent){const p=o.templateContent.replaceAll("%SLOT%",b).replaceAll("%CLASSES%",w);return m(p)}return f`<div class=${et(v)}>${m(b)}</div>`},$=(Array.isArray(c)?c:[c]).reduce((l,u)=>({...l,[`${u.name}${T(s).getSuffixFromType(u.type)}`]:u.value}),{});if(!i?.x&&!i?.y&&!o?.title)return f`${h({...d,...$})}`;const F=l=>l?Array.isArray(l)?l.map(u=>({...u,values:u.values||n(u.name)})):[{...l,values:l.values||n(l.name)}]:[{}],W=F(i?.x),B=F(i?.y),C=(Array.isArray(c)?c:[c]).find(l=>l.type==="template")?.value,S=`uuid-${crypto.randomUUID()}`;return f`
      <style>
        ...
        ${o?.templateBackgrounds?.colors.map((l,u)=>{const b=v=>`${o?.templateBackgrounds?.colors.length}n + ${v+1}`;return o?.templateBackgrounds?.alternate==="y"?`
                .${S}.story-template tbody tr.template-row:nth-of-type(${b(u)}) td.template {
                  background: ${l};
                }
              `:`
                .${S}.story-template tbody tr.template-row td.template:nth-of-type(${b(u)}) {
                  background: ${l};
                }
              `})}
      </style>
      ${W.map(l=>f` ${B.map(u=>{let b=!0;const v=W.length>1||l.values,w=(l&&u||B.length>1)&&u?.values;return f`
            <table class="story-template ${S} ${o?.classes}">
              <thead>
                ${o?.title&&f`<tr>
          ...
          `})}`)}
    `}}},Vt=(s,e)=>it?.components?.[s]?.[e]?.description?.value??"No Description",Yt=(s,e=150)=>{const r=isNaN(e)?e:{heightPx:e},{afterRender:t="",additionalChromaticOptions:n={},heightPx:a=150}=r;return{parameters:{chromatic:{...nt?.parameters?.chromatic,...n,disableSnapshot:!1},docs:{disable:!0}},render:(i,c)=>f`
      ...
      `})}
      ${t}
    `}};export{It as a,Yt as b,T as c,Vt as g,A as s};
