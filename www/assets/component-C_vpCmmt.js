...
`;var sl=Object.defineProperty,rl=Object.getOwnPropertyDescriptor,On=(t,e,n,a)=>{for(var i=a>1?void 0:a?rl(e,n):e,o=t.length-1,s;o>=0;o--)(s=t[o])&&(i=(a?s(e,n,i):s(i))||i);return a&&i&&sl(e,n,i),i};let ll=0;class nn extends w{constructor(){super(...arguments),this.attrId=++ll,this.componentId=`syn-tab-panel-${this.attrId}`,this.name="",this.active=!1}static{this.styles=[x,al,ol]}connectedCallback(){super.connectedCallback(),this.id=this.id.length>0?this.id:this.componentId,this.setAttribute("role","tabpanel")}handleActiveChange(){this.setAttribute("aria-hidden",this.active?"false":"true")}render(){return c`
      <slot
        part="base"
        class=${f({"tab-panel":!0,"tab-panel--active":this.active})}
      ></slot>
    `}}On([r({reflect:!0})],nn.prototype,"name",2);On([r({type:Boolean,reflect:!0})],nn.prototype,"active",2);On([m("active")],nn.prototype,"handleActiveChange",1);nn.define("syn-tab-panel");const dl=h`
	/* stylelint-disable */
  :host {
    display: inline-block;
  }
...
`):a=e.innerHTML,a=a.replace(/<style><\/style>/g,"").replace(/<!-- preview-ignore:start -->[\s\S]*?<!-- preview-ignore:end -->/g,"").replace(/\/\/ preview-ignore:start[\s\S]*?\/\/ preview-ignore:end/g,"").replace(/<style>\n<\/style>/g,"").replace(/<script>\s*component = document\.querySelector\('(.+?)'\);\s*<\/script>/g,""),a}},Gl=(t,e,n="components")=>{const a=pa[n][t]??{},i=a[e]?.description?.value??"No Description",o=a[e]?.note?.value??"",s=p=>p.replace(/\n/g,"<br/>"),l=s(i),d=o?`<br/><br/><strong>👨‍💻 Additional developer Information:</strong><br>${s(o)}`:"";return`${l}${d}`},Yl=(t,e=150)=>{const n=isNaN(e)?e:{heightPx:e},{afterRender:a="",additionalChromaticOptions:i={},heightPx:o=150,styleHeading:s={}}=n,l=Object.entries(s).map(([d,p])=>`${d}: ${p};`).join(" ");return{parameters:{chromatic:{...qi?.parameters?.chromatic,...i,disableSnapshot:!1},docs:{disable:!0}},render:(d,p)=>C`
      ...
      `})}
      ${a}
    `}};export{Sl as a,ct as b,Kl as c,Yl as d,ql as e,Gl as g,Wl as h,_l as i,qi as p,hn as s};
