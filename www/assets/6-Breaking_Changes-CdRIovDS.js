import{l as f,j as r,M as b,a as v}from"./blocks-B_o2G781.js";import{useMDXComponents as w}from"./index-DP1WbzDr.js";import{r as E,d as x}from"./iframe-DEjDpRef.js";import"./preload-helper-Ct5FWWRu.js";const{STORY_CHANGED:T,SELECT_STORY:k}=__STORYBOOK_MODULE_CORE_EVENTS__,{global:S}=__STORYBOOK_MODULE_GLOBAL__,{makeDecorator:R,addons:u}=__STORYBOOK_MODULE_PREVIEW_API__;var C=Object.defineProperty,A=(e,n)=>{for(var s in n)C(e,s,{get:n[s],enumerable:!0})},I="links",{document:m,HTMLElement:M}=S;function H(e){let n={},s=(e[0]==="?"?e.substring(1):e).split("&").filter(Boolean);for(let t=0;t<s.length;t++){let o=s[t].split("=");n[decodeURIComponent(o[0])]=decodeURIComponent(o[1]||"")}return n}var c=e=>u.getChannel().emit(k,e),_=(e,n)=>new Promise(s=>{let{location:t}=m,o=H(t.search).id,i=e||o.split("--",2)[0],l=`/story/${f(i,n)}`,d=t.pathname.replace(/iframe\.html$/,""),g=`${t.origin+d}?${Object.entries({path:l}).map(h=>`${h[0]}=${h[1]}`).join("&")}`;s(g)}),y=e=>{let{target:n}=e;if(!(n instanceof M))return;let s=n,{sbKind:t,sbStory:o}=s.dataset;(t||o)&&(e.preventDefault(),c({kind:t,story:o}))},a=!1,N=()=>{a||(a=!0,m.addEventListener("click",y))},O=()=>{a&&(a=!1,m.removeEventListener("click",y))},L=R({name:"withLinks",parameterName:I,wrapper:(e,n)=>(N(),u.getChannel().once(T,O),e(n))}),D={};A(D,{decorators:()=>F});var F=[L];const B=`# Handling breaking changes between major versions
